...
class EthErc20Client{
  ...
  Future<String> sendEthereum(
    ...
    logger.d('sendEthereum(credt:$credt, address:$address, amountEth:$amountEth, gasLimit:$gasLimit, gasPriceInGwei:$gasPriceInGwei)');
    try {
  ...
  Future<String> sendNknToken(
    ...
    logger.d('sendNknToken(credt:$credt, address:$address, amountNkn:$amountNkn, gasLimit:$gasLimit, gasPriceInGwei:$gasPriceInGwei)');
    try {
      return _web3client.sendTransaction(
          credt,
  ...
  void listenTokenEvent(void callback(EthereumAddress from, EthereumAddress to, EtherAmount balance)) {
    _subscription ??= _web3client
      ...
      logger.i('onTokenEvent: $from sent ${balance.getValueInUnit(EtherUnit.ether)} NKN to $to');
